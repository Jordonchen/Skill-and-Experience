"""
最近复习了操作系统的基本知识，关于页面置换算法的原理，需要在以后多多加强了解。为了更好地理解，打算用python去实现当中一个比较重要的置换算法---LRU。LRU算法（least recently used）叫做最近最久未使用算法，顾名思义，就是把当一个缺页终端发生的时候，选择最久未被使用的那个页面，并淘汰之。
其依据的是程序的局部性原理，即在近一段时间内，如果某些页面被频繁访问，那么在将来的一段时间内，它们很有可能被再次频繁访问。反过来说，如果某些页面长时间未被
访问，那么在将来一段时间它们还有可能会长时间不被访问。


那么，LRU算法需要记录各个页面使用时间的先后顺序，开销大，不过可能实现的方法如下：
   可以设置一个活动页面栈，当访问某页面时，将此页面压入栈顶，然后考察栈内是否有与此相同的页号，若有，则抽出。当需要淘汰一个页面时，总是选择栈底的页面，因为
   它就是最近最久未被使用的页面。
